...
将本机 `%USERPROFILE%\.tauri\wonderland-assistant.key` 的**完整内容**保存为仓库 Actions Secret `TAURI_SIGNING_PRIVATE_KEY`。私钥留在仓库外，并备份到安全位置；不要提交或发送到讨论区。当前私钥没有口令，因此 `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` 可以留空。公钥已经写入 Tauri 配置。
...
此阶段由维护者审核目录 PR。SHA-256 将下载内容绑定到审核记录，不代表作者个人数字签名；若以后开放无需人工审核的发布流程，再引入发布者密钥签名。网络请求限制到固定 Pages 源和 GitHub Release URL，并限制重定向、超时与包大小。
